    ...
    <span v-else>
     ...
     <el-table-column label="Actions"
         ...
         >
          ...
          type="default"
          v-tooltip="`Save role`"
          v-if="(isEditting && currentRow == scope.$index) || (scope.$index == 0 && isEditting && scope.row.newRow) && scope.row.name"
 ...
 this.fetchRoles(this.$route.params.programId)
  ...
  watch: { 
    ...
    addUserToRoleStatus: {
     ...
     removeAdminRoleStatus: {
      handler() {
        if (this.removeAdminRoleStatus == 200 || this.removeAdminRoleStatus == 204) {
          this.$message({
