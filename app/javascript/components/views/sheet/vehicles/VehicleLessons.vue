...
export default {
  ...
  methods: {
    ...
    addLesson() {
      this.$router.push(
        `/programs/${this.$route.params.programId}/sheet/vehicles/${this.$route.params.vehicleId}/lessons/new`
      );
    },
    sort:function(s) {
    ...
    _isallowed(salut) {
        return this.checkPrivileges("VehicleLessons", salut, this.$route)
   ...
   filteredLessons() {
      ...
      unfiltered: {
       lessons:  this.vehicleLessons
        ...
        filtered : {
          lessons: this.vehicleLessons.filter(lesson => {
