<template>
  <div data-cy="issues">
    ...
    <div
      v-if="!loading"
      class="issues_show mx-3 mb-3 mt-1 py-1"
      @click.prevent="editIssue"
    >
      <div v-if="show">
        <div class="row">
          <div class="col-md-9">
          ...
          <div class="col-md-3 mt-2">
            <div class="t_actions my-3">
              <div>
                <span v-if="issue.watched == true">
                  <span v-tooltip="`On Watch`"
                    ><i class="fas fa-eye text-md" data-cy="on_watch_icon"></i
                  ></span>
                </span>
              </div>
              <div>
        ...
        <div v-if="fromView == 'watch_view'" class="mt-3 font-sm row">
          <div class="col-6">
            ...
            <ol class="pl-4">
              <li v-for="(subTask, index) in DV_issue.subTasks" :key="index">
                <span
                  class="btn btn-link btn-sm p-0 clickable"
                  @click="openSubTask(subTask)"
                  >{{ getTask(subTask).text }}</span
                >
              </li>
            </ol>
          </div>
          <div class="col-6">
            ...
            <ol class="pl-4">
              <li v-for="(subIssue, index) in DV_issue.subIssues" :key="index">
                <span
                  class="btn btn-link btn-sm p-0 clickable"
                  @click="openSubIssue(subIssue)"
                  >{{ getIssue(subIssue).title }}</span
                >
              </li>
            </ol>
          </div>
...
export default {
  ...
  methods: {
    ...mapMutations(["updateIssuesHash", "setTaskForManager"]),
  ...
  computed: {
    ...
    is_overdue() {
      return (
        this.DV_issue.progress !== 100 &&
        new Date(this.DV_issue.dueDate).getTime() < new Date().getTime()
      );
    },
    facility() {
    ...
    C_editForManager() {
      return (
        this.managerView.issue && this.managerView.issue.id == this.DV_issue.id
      );
    },
  },
  watch: {
