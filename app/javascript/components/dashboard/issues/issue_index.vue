      ...
      <div class="d-flex align-item-center font-sm justify-content-between mt-2 w-100">
       <div class="simple-select w-50 mr-1 font-sm">
        ...
        <div class="simple-select w-50">
          <multiselect v-model="C_facilityManagerIssueFilter" v-tooltip="`Flags`" :options="getAdvancedFilterOptions" track-by="name" label="name" :multiple="true" select-label="Select" deselect-label="Remove" :searchable="false" :close-on-select="true" :show-labels="false" placeholder="Filter by Flags">
            <template slot="singleLabel" slot-scope="{option}">
              <div class="d-flex">
                <span class='select__tag-name'>{{option.name}}</span>
              </div>
...
computed: {
  ...
  filteredIssues() {
    ...
    let issues = _.sortBy(_.filter(this.facility.issues, ((resource) => {
      ...
      valid = valid && this.filterDataForAdvancedFilter([resource], 'facilityManagerIssues')
