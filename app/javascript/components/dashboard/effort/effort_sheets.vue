  ...
  <div id="notes-index" data-cy="note_list" class="mt-5 pl-1" :load="log(weekOfArr)">
    <span :load="log(editableTabsValue)">
      <div class="form-group w-100 mr-1 row">
        ...
        <div class="col-2 mt-1 pr-0">
          ...
          <el-select v-model="weekBegin" class="mr-1 d-flex" track-by="id" value-key="id" clearable
        ...
        <div class="col-2 mt-1 pl-0">
          ...
          <el-select v-model="weekEnd" class="d-flex" track-by="id" value-key="id" clearable
            placeholder="Select Week of End" filterable>
    ...
    <el-tabs type="border-card" v-model="editableTabsValue">
      <el-tab-pane label="Summary" class="is-active">
        <el-table v-if="tableData && matrixDates && matrixDates.length > 0" :data="tableData" height="450"
          class="crudRow mt-4" :header-row-style="{ textAlign: 'center' }">
        ...
        <div v-else>
          You currently have no Tasks in this Project or the Task due date has passed. Please add a task or ensure the due
      ...
      <el-tab-pane v-for="(item, index) in efforts" :key="item.id" :label="item.full_name" :name="index">
        ...
        <el-table v-if="tableData && matrixDates && matrixDates.length > 0" :data="tableData" height="450" id="crudRow"
          class="crudRow mt-4" :header-row-style="{ textAlign: 'center' }">
          <el-table-column prop="plannedEffort" label="Planned Effort" width="80" header-align="center">
          </el-table-column>
              ...
              <template slot-scope="scope">
                ...
                <span v-if="rowId == scope.row.id">
                  <span v-if="item.tasks && item.tasks.length > 0 && getWeekOfEffort(item.tasks, scope.row, weekof)">
                    <input :value="getWeekOfEffort(item.tasks, scope.row, weekof)" :name="weekof" type="text" class="form-control" />
                    <!-- CREATE MODE IF ENTER/EDIT BUTTON IS CLICKED -->
                  </span>

                  <input v-else v-model="input[weekofIndex]" :name="weekof" type="text" class="form-control" :id="weekof" />
                </span>
              </template>
            </el-table-column>
          </el-table-column>
          ...
          <el-table-column label="Actions" width="110" align="center">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="default" v-tooltip="`Add Effort`" class="bg-light px-2"
              ...
              <el-button type="default" size="mini" @click.prevent="saveEffortRow(scope.$index, scope.row, item.id)"
...
export default {
  ...
  methods: {
    ...
    getWeekOfEffort(_tasks, _row, _weekof){
      ...
      let hour = _tasks.filter(t => t.id == _row.id)
                      ...
                      .map(t => t.hours)[0]
      console.log('getWeekOfEffort', _row, _weekof, hour)
      return hour;
    },
