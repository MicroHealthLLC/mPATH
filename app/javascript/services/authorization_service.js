  ...
  getRolePrivileges: () => {
    axios({
      ...
      .then((res) => {
        ...
        AuthorizationService.programSettingPrivilegesRoles = res.data.program_settings_privileges_roles;
      })
      .catch((err) => {
        console.log(err);
      })
  ...
  checkPrivileges: (page, salut, route, extraData) => {
    ...
    if(["portfolio_issue_form", "KanbanIssues", "issue_sheets_index", "issue_index", "issue_calendar", "issue_form"].includes(page) ){
      ...
      if(extraData["method"] == "isallowedProgramSettings"){
        ...
        let contract_privileges = AuthorizationService.contractPrivilegesRoles[extraData["project_contract_id"]]  
