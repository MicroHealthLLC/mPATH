ActiveAdmin.register TaskStage do
  ...
  breadcrumb do
    links = [link_to('Admin', admin_root_path), link_to('Task Stages', admin_task_stages_path)]
    if %(show edit).include?(params['action'])
      links << link_to(task_stage.name, edit_admin_task_stage_path)
    end
    links
  end
  ...
  batch_action :destroy, if: proc {current_user.admin_delete?}, confirm: "Are you sure you want to delete these Task Stages?" do |ids|
    deleted = TaskStage.where(id: ids).destroy_all
    redirect_to collection_path, notice: "Successfully deleted #{deleted.count} Task Stages"
  end
  ...
  controller do
    ...
    def check_order
      ...
      redirect_to admin_task_stages_path(order: order) and return unless params[:order] == order
    end
