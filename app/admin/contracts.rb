ActiveAdmin.register Contract do
  ...
  index do
    ...
    column :contract_vehicle, nil, sortable: 'contracreturn unless params[:issue][:file_links]t_vehicles.name' do |contract|
      if current_user.admin_write?
  ...
  controller do
    ...
    def handle_files
      ...
      params[:contract].delete(:issue_files)
    end

    def handle_links
      resource.manipulate_links(params) if resource.present?
      params[:contract].delete(:file_links)
    end
