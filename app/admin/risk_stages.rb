ActiveAdmin.register RiskStage do

  menu parent: "Risks", priority: 2
  ...
  breadcrumb do
    links = [link_to('Admin', admin_root_path), link_to('Risk Stages', admin_risk_stages_path)]
    if %(show edit).include?(params['action'])
      links << link_to(risk_stage.name, edit_admin_risk_stage_path)
    end
    links
  end
  ...
  batch_action :destroy, if: proc {current_user.admin_delete?}, confirm: "Are you sure you want to delete these Risk Stages?" do |ids|
    deleted = TaskStage.where(id: ids).destroy_all
    redirect_to collection_path, notice: "Successfully deleted #{deleted.count} Risk Stages"
  end
