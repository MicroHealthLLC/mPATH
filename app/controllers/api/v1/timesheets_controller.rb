class Api::V1::TimesheetsController < AuthenticatedController 
  ...
  def index
    ...
    all_timesheets = Timesheet.includes([ :user, {facility_project: :facility} ]).where("timesheets.facility_project_id = ?", "timesheets.hours > 0", @owner.id)#.paginate(:page => params[:page], :per_page => 15)
