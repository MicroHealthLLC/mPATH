describe('Admin Panel Issue Severities', function() {
  ...
  it('Delete Issue Severity', function() {
    ...
    //cy.get('#issue_severity_submit_action').contains('Create Issue severity').click()
    cy.get('#q_name').type('1 New Test Issue Severity').should('have.value', '1 New Test Issue Severity')
  ...
  it('Search Issue Severity contains name', function() {
    ...
    cy.get('#issue_severity_name').type('2 New Test Issue Severity').should('have.value', '2 New Test Issue Severity')
    cy.get('#issue_severity_submit_action').contains('Create Issue severity').click()
    cy.get('#q_name').type('2 New Test Issue Severity').should('have.value', '2 New Test Issue Severity')
    cy.get('[type=submit]').first().contains('Filter').click()
    ...
    cy.get('.current_filter').contains('Name contains 2 New Test Issue Severity').should('be.visible')
    cy.get('#index_table_issue_severities > tbody > tr').its('length').should('be.eq', 1)
